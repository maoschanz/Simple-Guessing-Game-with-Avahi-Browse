...
/**
 * Generates a random number between minimum value and maximum value, uses the
 * portnumber (generated by the system) as seed
 */
...
/**
 * Parses the number out of a string ensuring both a bounds check and filtering
 * of any non-numerical values.
 * ASSUMPTION: The string contained in *kuchi has an instance of "guess" in some
 * form or mixture
 */
int get_number(char * kuchi, int i, int *f) {
	int NI;

	// traverse the pointer the whole length of guess, we're only interested in
	// what comes after guess
...
void handle_client(int i, int port__NO, fd_set * masta) {
	...
	// closes connection and clears file descriptor of socket from the master
	// (duplicate) file descriptor set
	close(i);
	FD_CLR(i, masta);
}
	...
	while(1) {
		...
		for(int i = 0; i <= max_file_descp && ret > 0; i++) {
			if(FD_ISSET(i, &fdset)) {
				...
				} else if(i == listn_file_descp) {
					// ^ something has been found on the listening socket

					// `accept` gives new port where this new client is found
					commu_file_descp = accept(
						listn_file_descp,
